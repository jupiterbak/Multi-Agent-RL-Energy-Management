...
class EFlexMultiAgentCompetitionClsV2(gym.Env):
    ...
    def _import_module(module_name, class_name):
        ...
        module = getattr(module0, 'EFlexMultiAgentCompetitionV2')
    ...
    def step(self, action):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if agent.current_state.value == EFLEXAgentState.Completed.value and last_agent_state == EFLEXAgentState.Execute.value:
        ...
        if len(self.current_winner) > 0:
            for i in range(self.n):
                if i in self.current_winner:
                    reward_n[i] = reward_n[i] + 1.0
            # self.global_reward = 1.0
            # done = True
    ...
    def render(self, mode='human', close=False):
        tmp = '\t|\t'.join('{:<10} - State: {:<12} - Reward: {:.2f} - PC: {:.2f}'.format(agent.name,
                                                                              agent.current_state.name,
                                                                              agent.current_reward,
                                                                              agent.production_count if hasattr(agent, 'production_count') else 0.0) for
                           i, agent in enumerate(self.agents))
        print('POWER: {:>6}%\t|\t WINNER: {:>6}\t|\t{}'.format(self.current_system_power, str(self.current_winner), tmp))
...
class EFlexAgent:
    ...
    def _get_done(self):
        return self.current_state == EFLEXAgentState.Aborted or self.production_count >= MAX_PRODUCTION_STEP
