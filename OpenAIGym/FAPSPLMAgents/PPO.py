...
class PPO(object):
    ...
    def add_experiences(self, observation, action, next_observation, reward, done, info):
        ...
        self.replay_memory.append(
            (observation, action, next_observation, reward, done, info, self.last_prediction, None))

        # Process the reward if done
        if done:
            # find and extract all unprocessed reward
            index = 0
            _len_mem = len(self.replay_memory)
            for state, action, next_state, reward, done, info, last_pred, pro_reward in self.replay_memory:
                if pro_reward is None:
                    break
                index = index + 1
            _list = [self.replay_memory.pop() for _i in range(index, _len_mem)]
            _list.reverse()

            # process the rewards
            reward_batch = []
            for state, action, next_state, reward, done, info, last_pred, pro_reward in _list:
                reward_batch.append(reward)
            reward_batch = self.transform_reward(reward_batch)

            # write it back to the memory
            _i = 0
            for state, action, next_state, reward, done, info, last_pred, pro_reward in _list:
                self.replay_memory.append(
                    (state, action, next_state, reward, done, info, last_pred, reward_batch[_i]))
                _i += 1
        pass
    ...
    def process_experiences(self, current_info, action_vector, next_info):
        ...
        """
        # Nothing to be done in the PPO case
    ...
    def update_model(self):
        ...
        actor_log = self.actor_model.train_on_batch([state0_batch, advantage_batch, old_prediction_batch],
                                                    action_batch, shuffle=True)
        train_names = ['actor_loss']
        self._write_log(self.tensorBoard, train_names, actor_log, int(self.steps / self.batch_size))

        # Train critic
        critic_log = self.critic_model.train_on_batch(state0_batch, reward_batch, shuffle=True)
