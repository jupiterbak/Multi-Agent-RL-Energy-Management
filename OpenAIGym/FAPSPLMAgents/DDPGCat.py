...
from keras.layers import Dense, add, Dropout, Lambda
...
class DDPGCat(object):
    ...
    def __str__(self):
        return '''Deep Deterministic Policy Gradient Trainer with Categorical Cross Entropy'''
    ...
    def _create_actor_model(self):
        ...
        # o = Lambda(lambda h: tf.contrib.distributions.RelaxedOneHotCategorical(0.5, probs=h).sample())(o)
    ...
    def initialize(self):
        ...
        self.target_model.compile(loss='categorical_crossentropy', optimizer=Adam(lr=self.learning_rate))
        print('\n##### Actor Target Model ')
    ...
    def load_model_and_restore(self, model_path):
        ...
        """
        self.actor_model, self.actor_trainable_weights, self.actor_input = self._create_actor_model()
        if os.path.exists('./' + model_path + '/DPPGCat_actor_model.h5'):
            self.actor_model.load_weights('./' + model_path + '/DPPGCat_actor_model.h5')
        ...
        if os.path.exists('./' + model_path + '/DPPGCat_actor_target_model.h5'):
            self.target_model.load_weights('./' + model_path + '/DPPGCat_actor_target_model.h5')
        self.target_model.compile(loss='mse', metrics=['mse'], optimizer=Adam(lr=self.learning_rate))

        self.critic_target_model, target_critic_state_input, target_critic_action_input = self._create_critic_model()
        if os.path.exists('./' + model_path + '/DPPGCat_critic_target_model.h5'):
            self.critic_target_model.load_weights('./' + model_path + '/DPPGCat_critic_target_model.h5')
        self.critic_target_model.compile(loss='mse', metrics=['mse'], optimizer=Adam(lr=self.learning_rate))

        self.critic_model, self.critic_state, self.critic_action = self._create_critic_model()
        if os.path.exists('./' + model_path + '/DPPGCat_critic_model.h5'):
            self.critic_model.load_weights('./' + model_path + '/DPPGCat_critic_model.h5')
        self.critic_model.compile(loss='mse', metrics=['mse'], optimizer=Adam(lr=self.learning_rate))
    ...
    def take_action(self, observation, _env):
        ...
        if len(self.replay_sequence) < self.time_slice - 1:
            ...
            if self.is_training:
                choice = np.random.choice(self.action_size, p=act_values[0])
                return choice
            else:
    ...
    def save_model(self, model_path):
        ...
        if os.path.exists('./' + model_path):
            self.actor_model.save('./' + model_path + '/DPPGCat_actor_model.h5')
            self.target_model.save('./' + model_path + '/DPPGCat_actor_target_model.h5')
            self.critic_model.save('./' + model_path + '/DPPGCat_critic_model.h5')
            self.critic_target_model.save('./' + model_path + '/DPPGCat_critic_target_model.h5')
        else:
